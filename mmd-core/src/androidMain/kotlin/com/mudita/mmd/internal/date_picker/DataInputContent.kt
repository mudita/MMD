...
internal fun DateInputContent(
    ...
    val errorDatePattern = context.getString(R.string.date_picker_input_date_not_match_pattern)
    val errorDateOutOfYearRange = context.getString(R.string.date_picker_input_date_out_of_range)
    val errorInvalidNotAllowed = context.getString(R.string.date_picker_input_date_not_allowed)
    ...
    val labelText = context.getString(R.string.date_picker_input_date)
    DateInputTextField(
        modifier = Modifier
            .fillMaxWidth()
...
internal fun DateInputTextField(
    ...
    // FIXME: Should be replaced with OutlinedTextFieldMMD when available.
    OutlinedTextField(
        value = text,
        onValueChange = { input ->
            if (
 ...
 * @param selectableDates a [SelectableDatesMMD] that is consulted to check if a date is allowed
...
internal class DateInputValidator(
    ...
    private val selectableDates: SelectableDatesMMD,
    private val dateInputFormat: DateInputFormat,
    private val dateFormatter: DatePickerFormatterMMD,
    ...
    /**
     * Validates a [CalendarDateMMD] input and returns an error string in case an issue with the given
     * date is detected, or an empty string in case there are no issues.
     *
     * @param dateToValidate a [CalendarDateMMD] input to validate
     ...
     */
    fun validate(
        dateToValidate: CalendarDateMMD?,
        inputIdentifier: InputIdentifier,
        locale: CalendarLocale,
    ): String {
        ...
        val option1 =
            inputIdentifier == InputIdentifier.StartDateInput && dateToValidate.utcTimeMillis >= (currentEndDateMillis ?: Long.MAX_VALUE)
        val option2 =
            inputIdentifier == InputIdentifier.EndDateInput && dateToValidate.utcTimeMillis < (currentStartDateMillis ?: Long.MIN_VALUE)
